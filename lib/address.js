...
function Address(blaver) {
  ...
  this.longitude = function (max, min, precision) {
    const localMax = max || 180;
    const localMin = min || -180;
    const localPrecision = precision || 4;

    return blaver.datatype
      .number({
        max: localMax,
        min: localMin,
        precision: parseFloat((0.0).toPrecision(localPrecision) + "1")
      })
      .toFixed(precision);
  };
  ...
  this.nearbyGPSCoordinate = function (coordinate, radius, isMetric) {
    const localRadius = radius || 10.0;
    const localIsMetric = isMetric || false;
