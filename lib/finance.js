...
var Finance = function (bluffmaster) {
  ...
  self.accountName = function () {
    return [
      Helpers.randomize(bluffmaster.definitions.finance.account_type),
      "Account",
    ].join(" ");
  };
  ...
  self.amount = function (min, max, dec, symbol, autoFormat) {
    ...
    symbol = symbol || "";
    const randValue = bluffmaster.datatype.number({
      max: max,
      min: min,
  ...
  self.currencyCode = function () {
    return bluffmaster.random.objectElement(
      bluffmaster.definitions.finance.currency
    )["code"];
  };
  ...
  self.currencySymbol = function () {
    ...
    while (!symbol) {
      symbol = bluffmaster.random.objectElement(
        bluffmaster.definitions.finance.currency
      )["symbol"];
    }
    return symbol;
  ...
  self.bitcoinAddress = function () {
    var addressLength = bluffmaster.random.number({ min: 27, max: 34 });

    var address = bluffmaster.random.arrayElement(['1', '3']);

    for (var i = 0; i < addressLength - 1; i++)
      address += bluffmaster.random.alphaNumeric().toUpperCase();
  ...
  self.litecoinAddress = function () {
    ...
    for (var i = 0; i < addressLength - 1; i++)
      address += bluffmaster.random.arrayElement(
