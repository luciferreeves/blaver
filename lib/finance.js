...
const Finance = function (blaver) {
  ...
  this.currencyCode = function () {
    return blaver.random.objectElement(blaver.definitions.finance.currency)["code"];
  };
  ...
  this.currencyName = function () {
    return blaver.random.objectElement(blaver.definitions.finance.currency, "key");
  };
  ...
  this.currencySymbol = function () {
    ...
    while (!symbol) {
      symbol = blaver.random.objectElement(blaver.definitions.finance.currency)["symbol"];
    }
  ...
  this.creditCardNumber = function (provider = "") {
    let format, formats;
    const localFormat = blaver.definitions.finance.credit_card;
    if (provider in localFormat) {
      formats = localFormat[provider]; // there chould be multiple formats
      ...
      if (typeof localFormat === "string") {
        format = localFormat;
      } else if (typeof localFormat === "object") {
        // Credit cards are in a object structure
        formats = blaver.random.objectElement(localFormat, "value"); // There chould be multiple formats
        if (typeof formats === "string") {
          format = formats;
        } else {
          format = blaver.random.arrayElement(formats);
