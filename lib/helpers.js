...
const Helpers = function (blaver) {
   ...
   * @param {array} array
   */
  self.randomize = function (array = ["a", "b", "c"]) {
   ...
   * @param {number} num
   */
  self.repeatString = function (string, num = 0) {
    let text = "";
    for (let i = 0; i < num; i++) {
  ...
  self.regexpStyleStringParse = function (string = "") {
    ...
    let token = localString.match(RANGE_REP_REG);
    while (token !== null) {
      ...
      localString =
        localString.slice(0, token.index) +
        blaver.helpers.repeatString(token[1], repetitions) +
        localString.slice(token.index + token[0].length);
      token = localString.match(RANGE_REP_REG);
    }
    // Deal with repeat `{num}`
    token = localString.match(REP_REG);
    while (token !== null) {
    ...
    while (token !== null) {
      ...
      localString =
        ...
        localString.slice(token.index + token[0].length);
      token = localString.match(RANGE_REG);
    }
   ...
   * @param {array} o
   */
  self.shuffle = function (options = []) {
    let localOptions = options || ["a", "b", "c"];
    for (let x, j, i = o.length - 1; i > 0; --i) {
      j = blaver.datatype.number(i);
      x = localOptions[i];
      localOptions[i] = localOptions[j];
      localOptions[j] = x;
    }
  ...
  */
  ...
  self.mustache = function (str = "", data) {
    ...
    for (const p in data) {
      ...
      localStr = str.replace(re, data[p]);
    }
  ...
  self.contextualCard = function () {
    ...
    const userName = blaver.internet.userName(name);
