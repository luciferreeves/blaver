...
var Helpers = function (bluffmaster) {
  ...
  self.replaceSymbols = function (string) {
    ...
    for (var i = 0; i < string.length; i++) {
      if (string.charAt(i) == "#") {
        ...
        str += bluffmaster.datatype.boolean()
          ? bluffmaster.random.arrayElement(alpha)
  ...
  self.regexpStyleStringParse = function (string) {
    ...
    while (token !== null) {
      ...
      string =
        ...
        bluffmaster.helpers.repeatString(token[1], repetitions) +
   ...
   * @example uniqueArray(bluffmaster.random.word, 50)
   * @example uniqueArray(bluffmaster.definitions.name.first_name, 6)
   * @example uniqueArray(["Hello", "World", "Goodbye"], 2)
   * @method bluffmaster.helpers.uniqueArray
  ...
  */
   self.uniqueArray = function(source, length) {
    if (Array.isArray(source)) {
      ...
      return bluffmaster.helpers.shuffle(array).splice(0, length);
    }
    const set = new Set();
    try {
  ...
  self.createCard = function () {
    return {
      ...
      accountHistory: [
        bluffmaster.helpers.createTransaction(),
        bluffmaster.helpers.createTransaction(),
        bluffmaster.helpers.createTransaction(),
      ],
    };
  ...
  self.createTransaction = function () {
    return {
      ...
      name: [
        bluffmaster.finance.accountName(),
        bluffmaster.finance.mask(),
      ].join(" "),
