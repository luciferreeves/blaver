...
const Lorem = function (blaver) {
   ...
   */
  this.sentence = function (wordCount = blaver.datatype.number({ min: 3, max: 10 })) {
   ...
   */
  this.sentences = function (sentenceCount = blaver.datatype.number({ min: 2, max: 6 }), separator = " ") {
    const sentences = [];
   ...
   */
  this.paragraph = function (sentenceCount = 3) {

    return blaver.lorem.sentences(sentenceCount + blaver.datatype.number(3));
  };
   ...
   * @param {string} separator defaults to `'\n \r'`
   */
  this.paragraphs = function (paragraphCount = 3, separator = "\n \r") {
    const paragraphs = [];
   ...
   */
  this.lines = function lines(lineCount = blaver.datatype.number({ min: 1, max: 5 })) {
