...
const arrayRemove = function (arr, values) {
  ...
  values.forEach(function (value) {
    localArr = localArr.filter(function (ele) {
      return ele !== value;
    });
  });
...
function Random(blaver, seed) {
  ...
  this.number = function (options) {
    console.log("Deprecation Warning: blaver.random.number is now located in blaver.datatype.number");
  ...
  this.float = function (options) {
    console.log("Deprecation Warning: blaver.random.float is now located in blaver.datatype.float");
   ...
   * @param {array} array
   */
  this.arrayElement = function (array = ["a", "b", "c"]) {
   ...
   */
  this.arrayElements = function (array = ["a", "b", "c"], count) {
    let localCount = count;
    if (typeof localCount !== "number") {
      localCount = blaver.datatype.number({ min: 1, max: array.length });
    } else if (localCount > array.length) {
    ...
    let i = array.length;
    const min = i - localCount;
    let temp;
    let index;

    while (i-- > min) {
      index = Math.floor((i + 1) * blaver.datatype.float({ min: 0, max: 0.99 }));
   ...
   * @param {mixed} field
   */
  this.objectElement = function (object = { foo: "bar", too: "car" }, field) {
   ...
   */
  this.uuid = function () {
    console.log("Deprecation Warning: blaver.random.uuid is now located in blaver.datatype.uuid");
  ...
  this.boolean = function () {
    console.log("Deprecation Warning: blaver.random.boolean is now located in blaver.datatype.boolean");
   ...
   */
  this.words = function randomWords(count = blaver.datatype.number({ min: 1, max: 3 })) {
    const words = [];
  ...
  this.alpha = function alpha(options) {
    ...
    } else if (typeof localOptions === "number") {
      localOptions = {
        count: localOptions
      };
    } else if (typeof localOptions.count === "undefined") {
      localOptions.count = 1;
    }

    if (typeof localOptions.upcase === "undefined") {
      localOptions.upcase = false;
    }
    if (typeof localOptions.bannedChars === "undefined") {
      localOptions.bannedChars = [];
    }
    ...
    if (localOptions.bannedChars) {
      charsArray = arrayRemove(charsArray, localOptions.bannedChars);
    }
    for (let i = 0; i < localOptions.count; i++) {
      wholeString += blaver.random.arrayElement(charsArray);
    }

    return localOptions.upcase ? wholeString.toUpperCase() : wholeString;
  };
   ...
   */
  this.alphaNumeric = function alphaNumeric(count = 1, options = {}) {
  ...
  this.hexaDecimal = function hexaDecimal(count) {
    console.log("Deprecation Warning: blaver.random.hexaDecimal is now located in blaver.datatype.hexaDecimal");
