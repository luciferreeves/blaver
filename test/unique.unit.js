...
describe("unique.js", function () {
  describe("unique()", function () {
    ...
    it("is able to call a function with no arguments and return a result", function () {
      var result = bluffmaster.unique(bluffmaster.internet.email);
      assert.strictEqual(typeof result, 'string');
    });

    it("is able to call a function with arguments and return a result", function () {
      var result = bluffmaster.unique(bluffmaster.internet.email, ['a', 'b', 'c']); // third argument is provider, or domain for email
    ...
    it("is able to exclude results as array", function () {
      var result = bluffmaster.unique(bluffmaster.internet.protocol, [], { exclude: ['https'] });
    ...
    it("is able to limit unique call by maxTime in ms", function () {
      var result;
      try {
        result = bluffmaster.unique(bluffmaster.internet.protocol, [], { maxTime: 1, maxRetries: 9999, exclude: ['https', 'http'] });
      } catch (err) {
    ...
    it("is able to limit unique call by maxRetries", function () {
      var result;
      try {
        result = bluffmaster.unique(bluffmaster.internet.protocol, [], { maxTime: 5000, maxRetries: 5, exclude: ['https', 'http'] });
      } catch (err) {
